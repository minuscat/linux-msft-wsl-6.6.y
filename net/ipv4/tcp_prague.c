...
static u32 prague_max_tso_segs __read_mostly; /* Static variable init to 0 */
...
static int prague_alpha_mode __read_mostly; /* Static variable init to 0 */
...
skip:
	ca->hsrtt_us = ca->hsrtt_us + tp->srtt_us -
...
static void prague_init(struct sock *sk)
{
	...
	if (tp->srtt_us) {
		ca->rate_bytes = div_u64(((u64)USEC_PER_SEC << 3) *
					 tcp_mss_to_mtu(sk, tp->mss_cache),
					 tp->srtt_us);
		ca->rate_bytes = max_t(u64, ca->rate_bytes * tcp_snd_cwnd(tp),
				       MINIMUM_RATE);
	} else {
